...
Now we add our v2 'canary' to the v1 cluster my mapping the v1 route to cities-ui app (i.e. the -n parameter is the host for cities-monolithic returned by 'cf routes')
...
Now if we repeatedly visit our production URL, we'll see the CF router is load balancing requests between the v1 and v2 apps ('cf routes' will show both apps mapped to this route).
...
Finally, with a bit of spare memory, you can try scaling both tiers of your v2 app.
